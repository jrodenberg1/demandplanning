...
@st.cache_data
def getDepartments(merchant_menu):
    ...
    query = f'''
   SELECT
	DISTINCT DEPARTMENT_MEMBER_NUMBER || '-' || DEPARTMENT_NAME AS DEPT ,
	DEPARTMENT_MEMBER_NUMBER
FROM
	EDW_SPOKE.NZ.DIM_PRODUCT_BPS PD
JOIN(
	SELECT
		DISTINCT MCHNUM,
		INUMBR
	FROM
		NZ.PRODUCT_BPS_INVMST
		WHERE MCHNUM IN ( '''
    for index in range(0, len(merchant_menu)-1):
        query += f''' '{merchant_menu[index]}','''
    query += f''' '{merchant_menu[-1]}' '''
    query += f''')) mst1 ON
	mst1.INUMBR = CAST(pd.SKU_DISPLAY_NUMBER AS INT)
WHERE
	SKU_DISPOSITION_CODE IN ('A',
	'N')
...
st.divider()
 ...
 #getting store number dropdown, if no selection then ALL stores
...
#getting merchant dropdown, if no selection then ALL merchants
merchant_menu = GetMerchant()

chosen_merchants = st.multiselect('Merchant(s):',merchant_menu)

if len(chosen_merchants) == 0: 
    chosen_merchants = merchant_menu

#getting dept and dropdown, if no selection then ALL depts
deptsMenu = getDepartments(chosen_merchants)
...
##start date/week and end_date
from datetime import date, timedelta

start_week = date.today().isocalendar()[1] - 26

start_date = date.today() - timedelta(weeks=start_week)

start = st.date_input("Start Date:",start_date)

end = st.date_input("Current Date:",date.today())
...
def main_query():
    ...
    query = f'''WITH PAST_FCST AS 
       ...
       P.SUB_CLASS_MEMBER_NUMBER AS SUB_CLASS_NUM,
                     SKUNBR,
                     STRNBR,
              PERCENTILE_CONT(.5) WITHIN GROUP (ORDER BY CASE WHEN SKUSDT >= {start} THEN TYFUNT END) AS PAST_FCST
              FROM
       ...
       MAX(P.STYLE_DISPLAY_NUMBER) AS STYLE,
                     ...
                     STRNBR ,
                     MAX(CASE WHEN SKUSDT > {start} THEN TYFUNT END) AS CURRENT_FCST,
...
AND R.SUB_NUM IN ( '''
    ...
    query += f''' '{stores[-1]}' '''
    query += f''')
    order by abs(R.FCST_CHANGE) DESC'''
...
st.write("Size of Data Pull:",df1.shape[0])